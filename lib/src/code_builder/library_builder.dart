...
import 'package:get_x_navigation_generator/src/code_builder/builder_utils.dart';
import 'package:get_x_navigation_generator/src/models/dependency_config.dart';
import 'package:get_x_navigation_generator/src/models/dispose_function_config.dart';
import 'package:get_x_navigation_generator/src/models/external_module_config.dart';
...
class LibraryGenerator with SharedGeneratorCode {
  ...
  Library generate() {
    ...
    final scopedDeps = groupBy<DependencyConfig, String?>(dependencies, (d) => d.scope);
    final scopedBeforeExternalModules = groupBy<ExternalModuleConfig, String?>(microPackagesModulesBefore, (d) => d.scope);
    final scopedAfterExternalModules = groupBy<ExternalModuleConfig, String?>(microPackagesModulesAfter, (d) => d.scope);
...
class InitMethodGenerator with SharedGeneratorCode {
  ...
  Method generate() {
    ...
    final useAsyncModifier = microPackagesModulesBefore.isNotEmpty || microPackagesModulesAfter.isNotEmpty || hasPreResolvedDependencies(dependencies);
    ...
    final ghStatements = [
      for (final pckModule in microPackagesModulesBefore.map((e) => e.module))
        refer(pckModule.name, pckModule.import).newInstance(const []).property('init').call([_ghLocalRefer]).awaited.statement,
      ...modules.map((module) => declareFinal(toCamelCase(module.type.name))
    ...
    return Method(
      (b) => b
        ..docs.addAll([
          if (!asExtension && scopeName == null && !isMicroPackage) ...['\n/// ignore_for_file: unnecessary_lambdas', '/// ignore_for_file: lines_longer_than_80_chars'],
