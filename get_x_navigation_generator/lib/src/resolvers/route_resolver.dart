...
class RouteResolver {
  ...
  List<GetXRouteConfig> resolve(ClassElement clazz) {
    ...
    final getXRouteAnnotations = _getXRouteChecker.annotationsOf(annotatedElement, throwOnUnresolved: false);
    return getXRouteAnnotations.map((getXRouteAnnotation) {
      ...
      final preventDuplicates = getXRoute.peek('preventDuplicates')?.boolValue ?? true;
      ...
      final transition = getXRoute.peek('transition')?.objectValue as Transition?;
      final transitionDuration = getXRoute.peek('transitionDuration')?.objectValue as Duration?;
      final participatesInRootNavigator = getXRoute.peek('participatesInRootNavigator')?.boolValue;
      final title = getXRoute.peek('title')?.stringValue;
      final maintainState = getXRoute.peek('maintainState')?.boolValue ?? true;
      final opaque = getXRoute.peek('opaque')?.boolValue ?? true;
      final popGesture = getXRoute.peek('popGesture')?.boolValue;
      final showCupertinoParallax = getXRoute.peek('showCupertinoParallax')?.boolValue ?? true;
      ...
      return GetXRouteConfig(
        ...
        customTransition: customTransition == null ? null : _typeResolver.resolveType(customTransition),
